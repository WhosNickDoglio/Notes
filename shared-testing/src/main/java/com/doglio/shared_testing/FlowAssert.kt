 ...
 */
package com.doglio.shared_testing
...
/**
 * [TAKEN FROM HERE](https://github.com/cashapp/sqldelight/blob/master/extensions/coroutines-extensions/src/commonTest/kotlin/com/squareup/sqldelight/runtime/coroutines/FlowAssert.kt)
 *
 * Watch [this issue](https://github.com/cashapp/sqldelight/issues/1416)
 *
 */
    ...
    coroutineScope {
        ...
        val collectJob = launch {
            val terminalEvent = try {
                collect { item ->
                    events.send(com.doglio.shared_testing.Event.Item(item))
                }
                com.doglio.shared_testing.Event.Complete
            } catch (_: CancellationException) {
                null
            } catch (t: Throwable) {
                com.doglio.shared_testing.Event.Error(t)
            }
        ...
        val flowAssert = com.doglio.shared_testing.FlowAssert(events, collectJob, timeoutMs)
        val ensureConsumed = try {
            flowAssert.validate()
            true
        } catch (e: CancellationException) {
            if (e !== com.doglio.shared_testing.ignoreRemainingEventsException) {
                throw e
            }
            false
        }
