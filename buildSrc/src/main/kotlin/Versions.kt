object Versions {

    const val sqlDelight = "1.3.0"

    const val ktlintGradlePlugin = "9.2.1"

    const val benManesVersions = "0.28.0"

    const val delect = "0.2.1"

    const val daggerReflect: String = "0.2.0"

    const val scabbard = "0.2.0"

    const val kotlin: String = "1.3.72"

    const val navigation: String = "2.3.0-alpha06"

    const val fragment: String = "1.3.0-alpha04"

    const val dagger: String = "2.27"

    const val androidTest: String = "1.3.0-beta01"

    const val androidBuildTools: String = "3.6.3"

    const val junitTestExtensions: String = "1.1.2-beta01"

    const val truthTestExtensions: String = "1.3.0-beta01"

    const val truth: String = "1.0.1"

    const val junit: String = "4.13"

    const val detekt: String = "1.8.0"

    const val leakCanary: String = "2.2"

    const val constraintLayout: String = "2.0.0-beta4"

    const val espressoCore: String = "3.3.0-beta01"

    const val recyclerview: String = "1.2.0-alpha03"

    const val threetenabp: String = "1.2.4"

    const val appcompat: String = "1.2.0-beta01"

    const val material: String = "1.2.0-alpha06"

    const val cardview: String = "1.0.0"

    const val ktlint: String = "0.36.0"

    const val timber: String = "4.7.1"

    const val gradle: String = "6.4"

    const val coordinator: String = "1.1.0"

    const val threeten: String = "1.4.4"

    const val preferences: String = "1.1.1"

    const val coroutines: String = "1.3.5"

    const val flowBinding: String = "0.11.1"

    const val flowPreferences: String = "1.1.1"

    const val flipper: String = "0.41.0"

    const val soloader: String = "0.9.0"

    const val about = "8.1.2"

    const val junitJacoco = "0.16.0"

    const val lifecycle = "2.3.0-alpha02"
}
